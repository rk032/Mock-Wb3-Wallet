...
class WalletManager:
    ...
    def generate_mnemonic(self):
        return self.mnemo.generate(strength=128)  # 12 words
    
    def from_mnemonic(self, mnemonic):
        if not self.mnemo.check(mnemonic):
            raise ValueError("Invalid mnemonic")
        seed = self.mnemo.to_seed(mnemonic)
        account = Account.from_key(seed[:32])  # Simple derivation for mock
    ...
    def sign_message(self, private_key_hex, message):
        ...
        signed = account.sign_message(encode_defunct(text=message))  # Fixed: Use encode_defunct function
