...
class TransactionService:
    ...
    def get_eth_from_usd(self, usd_amount):
        ...
        # USDC denom for Ethereum: 0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48 (6 decimals)
        body = {
            "source_asset_denom": "0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48",
            "source_asset_chain_id": "1",
            "dest_asset_denom": "ethereum-native",
            "dest_asset_chain_id": "1",
            "amount_in": int(usd_amount * 1_000_000),  # USDC 6 decimals
        ...
        try:
            ...
            data = response.json()
            # Response has 'route' with 'amount_out' in wei (18 decimals for ETH)
    ...
    def verify_signature(self, message, signature_hex, sender_address):
        try:
            ...
            return recovered.lower() == sender_address.lower()  # Fixed: recovered is str (address), no .address
        except Exception as e:
            print(f"Debug - Recovery exception: {e}")  # Keep temporarily if needed
            return False
    ...
    def process_transfer(self, sender, recipient, amount, usd, signature, message):
        ...
        # For USD, re-check price
        if usd > 0:
            new_eth, _ = self.get_eth_from_usd(usd)
            if new_eth and abs(new_eth - amount) / amount > 0.01:  # 1% tolerance
        ...
        self.db.add_transaction(recipient, sender, recipient, amount, "received")  # For history
